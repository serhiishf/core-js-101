...
/*   const rect1x = rect1.left - rect1.width;
  ...
  return (rect1y < rect2y0 || rect1y0 > rect2y || rect1x0 < rect2x || rect1x > rect2x0); */
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr.filter((char) => char === arr[i]).length < 2) {
      if (arr[i] !== ' ') {
        return arr[i];
      }
    }
...
function isCreditCardNumber(ccn) {
  ...
  if (ccnStr.length % 2 !== 0) {
    start = 1;
    result += Number(ccnStr[0]);
  }
  for (let i = start; i < ccnStr.length; i += 2) {
    let temp = Number(ccnStr[i]) * 2;
    if (temp > 9) {
      temp -= 9;
    }
    result += temp + Number(ccnStr[i + 1]);
  }
  if (result % 10 === 0) {
    return true;
  }
